...
import {
  ...
  DialogDescription,
  DialogFooter,
} from "@/components/ui/dialog"; // Import các thành phần Dialog
  ...
  return (
    <div className="container p-5">
      ...
      <div className="flex justify-between border-b items-center">
        <h3 className="text-2xl font-medium mb-8 text-gray-800 mt-5">
          Địa chỉ của tôi
        </h3>
        <div className="flex items-center px-4 text-white bg-[#b8cd06] gap-2">
          <svg
            ...
            x={0}
            y={0}
            className="shopee-svg-icon w-[14px] icon-plus-sign"
      ...
      <div>
        <h3 className="text-2xl font-medium mb-1 text-gray-800 mt-5">
          Địa chỉ
        </h3>
        ...
        {isLoading ? (
          ...
          <div>
            ...
            {sortedAddresses && sortedAddresses.length > 0 ? (
              sortedAddresses.map((address: Address, index: string) => (
                <div key={index} className="flex py-5 border-b justify-between">
                  <div>
                    ...
                    {address.isDefault && (
                      <div className="mt-3">
                        <span className="border px-5 bg-[#b8cd06] text-white py-2">
                          Mặc định
                        </span>
                      </div>
                    )}
                  ...
                  <div>
                    <div className="flex items-center justify-between">
                      {!address.isDefault && (
                        <div
                          className="text-blue-500 float-right cursor-pointer mb-4"
                      ...
                      <Link
                        to={``}
                        className="text-blue-500 float-right mb-4"
                        onClick={() => handleEditClick(address._id)}
                      >
                        <EditAddress addressId={selectedAddressId} />
                      </Link>
                    </div>
                    <Button onClick={() => handledefault(address)}>
                      Thiết lập mặc định
                    </Button>
                  </div>
                </div>
              ...
              <p className="text-[red] h-44 flex justify-center items-center">
                Chưa có địa chỉ nào.
              </p>
            )}
          </div>
