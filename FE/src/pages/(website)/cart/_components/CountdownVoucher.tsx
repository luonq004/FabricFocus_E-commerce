    ...
    <div className="w-full flex flex-col gap-2">
      ...
      <div className="flex flex-col max-h-[400px] overflow-y-auto gap-4">
        {data?.map((item: any) => {
          ...
          const matchedVoucher = cart?.voucher.find(
            (voucher: any) => voucher._id === item.voucher._id
          );
          // kiểm tra xem voucher đã được sử dụng chưa
          const matchedVoucherUsage = voucherUsage?.find(
            (voucher: any) => voucher.voucherId === item.voucher._id
          );
          // kiểm tra xem voucher có trong giỏ hàng không
          const matchedVoucherCart = cart?.voucher.find(
            (voucher: any) => voucher._id === item.voucher._id
          );
          ...
          if (!isVoucherActive && matchedVoucherCart) {
            ...
            return (
              <div
                key={item.voucher._id}
                className={`voucher-item p-3 w-full grid grid-cols-[15%_auto_5.5%] gap-x-3 transition-all duration-200 border rounded-md ${
                  matchedVoucher ? "border-black" : "border-gray-300"
                }`}
              >
                <div className="bg-slate-300 flex justify-center items-center p-1">
                  {item.voucher.category === "product" ? (
                ...
                <div
                  className={`${
                    matchedVoucherUsage ? "hidden" : "flex"
