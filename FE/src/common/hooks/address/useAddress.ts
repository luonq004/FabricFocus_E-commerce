...
} from "@/pages/(website)/services/address/Address";
import { useQuery } from "@tanstack/react-query";

// Hàm UseCountry nhận cả userId và addressId
const useAddress = (userId?: string | null, addressId?: string) => {
  // Sử dụng React Query để lấy dữ liệu
  const { data, ...rest } = useQuery({
    queryKey: ["ADDRESS", userId, addressId], // Cập nhật key để gồm cả userId và addressId
    queryFn: async () => {
      if (userId) {
